...
func compiler_next(compiler_state initial) compiler_state {
    ...
    if c == "\"" {
        escaped := false
        for s.pos < n {
            next_char := __host_char_at(s.source, s.pos)
            s.pos = s.pos + 1
            if escaped {
                escaped = false
            } else if next_char == "\\" {
...
func compiler_parse_struct_decl(compiler_state initial) compiler_state {
    ...
    struct_id := s.struct_count
    field_start := s.function_param_total
    field_count := 0
    typedef_code := "typedef struct S_" + struct_name + " {\n"
    make_params := ""
    make_args := ""
    make_body := ""
    fields_drop := ""
...
func compiler_conflict_at(compiler_state initial, int owner, int field, bool exclusive) bool {
    ...
    i := 0
    for i < s.count {
        same_field := field < 0 || s.loan_fields[i] < 0 || s.loan_fields[i] == field
        mutable_loan := compiler_is_mutable_borrow_kind(s.kinds[i]) || s.kinds[i] == 11
        shared_loan := s.kinds[i] == 3 || s.kinds[i] == 10 || s.kinds[i] == 18
...
func compiler_atom_inner(compiler_state initial) compiler_state {
    ...
    if compiler_ident(t) && compiler_find_func(s, t) >= 0 {
        ...
        args := ""
        evaluations := ""
        call_id := compiler_number(s.pos)
        arg := 0
        loan_floor := s.count
        returned_loan_slot := -1
        for s.error == "" && s.token != ")" && s.token != "" {
            ...
            if expected == 7 || expected == 8 || expected == 15 || expected == 16 {
                if s.value_slot < 0 { return compiler_fail(s, "array argument requires a named array") }
                exclusive := expected == 8 || expected == 16
    ...
    if s.token == "[" {
        ...
        index := s.value
        s = compiler_expect(s, "]")
        data := compiler_var(slot)
        length := compiler_array_length(s, slot)
    ...
    if s.token == "." {
        ...
        if s.token == "." {
            ...
            s = compiler_next(s)
            nested_field := compiler_field_index(s, field_struct, s.token)
...
func compiler_expression(compiler_state initial, int minimum) compiler_state {
    ...
    for s.error == "" && compiler_precedence(s.token) >= minimum {
        op := s.token
        precedence := compiler_precedence(op)
        left := s.value
        temporary := "s_left" + compiler_number(s.pos)
...
func compiler_statement(compiler_state initial) compiler_state {
    ...
    if s.token == "if" {
        ...
        true_constant := compiler_extract_int64_constant(yes.value)
        if yes.terminated != 0 && no.terminated == 0 && branch_condition != "" && true_constant != "" {
            no.pending_branch_condition = branch_condition
            no.pending_branch_true_constant = true_constant
        }
    ...
    if s.token == "while" {
        old_floor := s.loop_floor
        s.loop_floor = s.count
        s = compiler_expression(compiler_next(s), 1)
        if s.value_kind != 1 { return compiler_fail(s, "condition requires an integer") }
        s.code = s.code + "while (" + s.value + ")\n"
        old_cleanup := s.loop_cleanup
    ...
    if s.token == "for" {
        ...
        step_name := s.token
        s = compiler_next(s)
        step_slot := compiler_find(s, step_name)
    ...
    if s.token == "return" {
        ...
        s = compiler_optional_semicolon(s)
        if s.pending_branch_condition != "" {
            false_constant := compiler_extract_int64_constant(s.value)
            if false_constant != "" {
                current_function := compiler_find_func(s, s.function_name)
                if current_function >= 0 {
                    s.function_branch_conditions[current_function] = s.pending_branch_condition
                    s.function_branch_true_constants[current_function] = s.pending_branch_true_constant
                    s.function_branch_false_constants[current_function] = false_constant
                }
            }
            s.pending_branch_condition = ""
            s.pending_branch_true_constant = ""
        }
        finish := "return compiler_result;"
    ...
    if s.token == "(" && compiler_find_func(s, name) >= 0 {
        function_index := compiler_find_func(s, name)
    ...
    if s.token == "." {
        ...
        target := compiler_var(slot) + "->" + compiler_struct_field_name(s, s.struct_ids[slot], field)
        replacement_type := compiler_c_type_for_kind(s, field_kind, field_struct)
...
func compiler_parse_helper(compiler_state initial) compiler_state {
    ...
    pi = 0
    for pi < param_count {
        ctype := compiler_c_type_for_kind(s, s.kinds[pi], s.function_param_structs[start + pi])
    ...
    function_index := compiler_find_func(s, name)
    if function_index >= 0 {
        s.function_return_constants[function_index] = compiler_extract_int64_constant(s.value)
        s.function_return_call_targets[function_index] = compiler_extract_zero_arg_call_target(s, s.value)
    }
    s.code = s.code + "}\n"
    return s
}
...
func compiler_compile(string source) compiler_state {
    ...
    function_return_constants := function_names
    function_return_call_targets := function_names
    function_branch_conditions := function_names
    function_branch_true_constants := function_names
    function_branch_false_constants := function_names
