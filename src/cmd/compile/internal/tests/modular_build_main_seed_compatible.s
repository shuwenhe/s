...
func main() int {
    ...
    if command == "build" {
        ...
        return compile.internal.backend_elf64.build(args[2], args[4], "", false)
    }
    if command == "check" {
        if len(args) != 3 {
            print_usage()
...
func run_tests(string fixtures_root) int {
    ...
    if compile.internal.tests.test_pipeline_regression.run_regression_suite() != 0 {
        std.io.eprintln("regression suite failed")
        return 1
    }
    std.io.eprintln("all test suites passed")
    return 0
}
