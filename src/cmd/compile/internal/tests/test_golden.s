...
func run_golden_suite(string fixtures_root) int {
    ...
    source_result := read_source(source_path)
    if source_result.is_err() {
        println("failed to read sample.s");
        return 1
    }
    token_result := tokenize(source_result.unwrap())
    if token_result.is_err() {
        println("lexer error");
        return 1
    }
    actual := dump_tokens_text(token_result.unwrap())
    expected_result := read_to_string(tokens_path)
