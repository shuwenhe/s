...
func run_ssa_suite() int {
    ...
    hot_balanced := build_pass_delta_hot_summary("constfold=1,gvn=1", "constfold=1,gvn=2", -1)
    if !contains(hot_balanced, ",dominant=balanced") {
        return 1
    }
    hot_struct := build_pass_delta_hot_summary("constfold=6,gvn=4", "constfold=1,gvn=0", -1)
    if !contains(hot_struct, ",dominant=struct") {
        return 1
    }
    hot_value := build_pass_delta_hot_summary("constfold=1,gvn=0", "constfold=5,gvn=3", -1)
    if !contains(hot_value, ",dominant=value") {
        return 1
    }
    hot_forced_balanced := build_pass_delta_hot_summary("constfold=6,gvn=4", "constfold=1,gvn=0", 99)
    if !contains(hot_forced_balanced, ",margin=99,dominant=balanced") {
        return 1
    }
    tie_reason := instruction_verify_pick_reason("shape", "cfg=1,rerun=1", "cfg")
    if tie_reason != "tie-break" {
        return 1
    }
    fallback_reason := instruction_verify_pick_reason("shape", "cfg=2,rerun=1", "rerun")
