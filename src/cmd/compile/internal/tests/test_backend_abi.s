...
func validate_emitted_artifacts(string out_path) bool {
    opt := require_artifact_markers(out_path + ".opt", vec[string]("midend-opt version=1", "scheduler_opt sroutine_sites=1", "select_timeout_sites=1", "select_send_sites=1"))
    if opt == "" || validate_midend_opt_artifact(opt).is_err() {
        return false
    }
    perf := require_artifact_markers(out_path + ".perf", vec[string]("perf-baseline version=1", "scheduler queue_policy=priority-rr select_policy=multi-chan-priority-rr", "select_timeout_sites=1", "select_send_sites=1", "scheduler_counters", "runtime_sched sroutine_scheduled=1", "runtime_gc cycles=", "heap_goal="))
    if perf == "" || validate_backend_perf_baseline(perf).is_err() {
        return false
    }
    toolchain := require_artifact_markers(out_path + ".toolchain", vec[string]("toolchain-compat version=1", "asm=go-plan9-min", "go_asm syntax=plan9 translator=enabled status=ok"))
    ...
    cfi := require_artifact_markers(out_path + ".cfi", vec[string]("cfi version=1", ".cfi_startproc", ".cfi_def_cfa", ".cfi_endproc"))
    if cfi == "" || validate_cfi_artifact(cfi).is_err() {
        return false
    }
    dwarf := require_artifact_markers(out_path + ".dwarf", vec[string]("section .debug_info", "section .debug_line", "section .debug_loc", "section .debug_ranges", "policy debug_budget_mode=", "metric location_continuity="))
