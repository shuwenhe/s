...
func linker_add_object_file(ctx* linker_context, string filename) int {
    obj := object_file {
        filename: filename,
...
func linker_resolve_symbols(ctx* linker_context) int {
...
func linker_apply_relocations(ctx* linker_context) int {
    for i := 0; i < ctx.object_files.len(); i = i + 1 {
        obj := ctx.object_files[i]
...
func linker_allocate_sections(ctx* linker_context) int {
...
func linker_write_executable(ctx* linker_context) link_result {
    result := link_result {
        success: 1,
        error_message: "",
