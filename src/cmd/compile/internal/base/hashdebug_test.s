...
func run_hashdebug_tests() int {
    hd_y := new_hash_debug("gossahash", "y")
    if !match_pkg_func(hd_y, "anything", "anyfunc") {
        return 1
    }
    hd_n := new_hash_debug("gossahash", "n")
    if match_pkg_func(hd_n, "anything", "anyfunc") {
        return 1
    }
    hd_empty := new_hash_debug("gossahash", "")
    if !match_pkg_func(hd_empty, "pkg", "fn") {
        return 1
    }
    hd_suffix := new_hash_debug("gossahash", "worker")
    ...
    hd_ex := new_hash_debug("gossahash", "worker/-bad")
