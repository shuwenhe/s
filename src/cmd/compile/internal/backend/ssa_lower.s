...
func make_ssa_to_machine(ctx* codegen_context,func* ssa_function) ssa_to_machine {
    tm: ssa_to_machine
    tm.ctx = ctx
    tm.func = func
...
func (m* ssa_to_machine) lower_block(b* ssa_block) {
    i := 0
    for i < len(b.values) {
        value_id := b.values[i]
...
func (m* ssa_to_machine) lower_value(int value_id) {
}

func (m* ssa_to_machine) emit_alloca(int size) string {
    offset := tm.ctx.alloc_state.next_stack_offset - size
    tm.ctx.alloc_state.next_stack_offset = offset
    "-" + to_string(-offset) + "(%rbp)"
}

func (m* ssa_to_machine) emit_load(string addr, int reg) {
    reg_name := x86_64_reg_name(reg)
    instr := "\tmovq\t" + addr + ", %" + reg_name
    tm.progs.append_prog(prog_op_load(), instr)
}

func (m* ssa_to_machine) emit_store(int reg, string addr) {
    reg_name := x86_64_reg_name(reg)
    instr := "\tmovq\t%" + reg_name + ", " + addr
    tm.progs.append_prog(prog_op_store(), instr)
}

func (m* ssa_to_machine) emit_binary_op(string op, int left_reg, int right_reg, int result_reg) {
...
func (m* ssa_to_machine) emit_comparison(string cond, int left_reg, int right_reg) int {
    left_name := x86_64_reg_name(left_reg)
    right_name := x86_64_reg_name(right_reg)
...
func (m* ssa_to_machine) generate() prog_list {
