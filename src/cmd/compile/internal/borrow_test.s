...
func run_borrow_checker_test() int {
    ...
    if compile.internal.borrow.borrow_check_events(shared_mut_conflict).ok {
        return 2
    }
    mutable_shared_conflict := string[] { "declare:x", "mutable:x", "shared:x" }
    if compile.internal.borrow.borrow_check_events(mutable_shared_conflict).ok {
        return 3
    }
    mutable_mut_conflict := string[] { "declare:x", "mutable:x", "mutable:x" }
    if compile.internal.borrow.borrow_check_events(mutable_mut_conflict).ok {
        return 4
    }
    move_conflict := string[] { "declare:text", "move:text", "read:text" }
    if compile.internal.borrow.borrow_check_events(move_conflict).ok {
        return 5
    }
    0
}
func run_lifetime_checker_test() int {
    ...
    if compile.internal.borrow.lifetime_check_events(dangling).ok {
        return 2
    }
    shorter_owner := string[] { "scope:outer", "scope:inner", "borrow:r:inner:outer" }
    if compile.internal.borrow.lifetime_check_events(shorter_owner).ok {
        return 3
    }
    ended_owner := string[] { "scope:outer", "end_scope:outer", "scope:inner", "borrow:r:outer:inner" }
    if compile.internal.borrow.lifetime_check_events(ended_owner).ok {
        return 4
    }
    ended := string[] { "scope:outer", "scope:inner", "borrow:r:outer:inner", "end_borrow:r", "use_ref:r" }
    if compile.internal.borrow.lifetime_check_events(ended).ok {
        return 5
    }
    0
}
func run_ownership_checker_test() int {
    ...
    if compile.internal.borrow.ownership_check_events(move_use_fail).ok {
        return 2
    }
    noncopy_fail := string[] { "declare:text:move", "copy:text" }
    if compile.internal.borrow.ownership_check_events(noncopy_fail).ok {
        return 3
    }
    double_drop_fail := string[] { "declare:file:move", "drop:file", "drop:file" }
    if compile.internal.borrow.ownership_check_events(double_drop_fail).ok {
        return 4
    }
