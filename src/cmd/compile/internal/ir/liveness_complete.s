...
func (liveness_analyzer* la) mark_use(block_id i32, value_id i32) {
...
func (liveness_analyzer* la) mark_def(block_id i32, value_id i32) {
    if block_id >= 0 && block_id < la.num_blocks && value_id >= 0 && value_id < la.num_values {
        found := false
...
func (liveness_analyzer* la) compute_liveness(succs i32[][]) {
    changed := true
    for changed {
        changed = false
...
func (liveness_analyzer* la) is_live_at_point(value_id i32, block_id i32, instr_index i32) bool {
...
func (liveness_analyzer* la) get_live_values(block_id i32) i32[] {
    i32[] result
...
func (liveness_analyzer* la) get_live_in(block_id i32) i32[] {
    i32[] result
    if block_id >= 0 && block_id < la.num_blocks {
        for v := i32(0); v < la.num_values; v += 1 {
...
func (liveness_analyzer* la) get_live_out(block_id i32) i32[] {
    i32[] result
    if block_id >= 0 && block_id < la.num_blocks {
        for v := i32(0); v < la.num_values; v += 1 {
...
func (liveness_analyzer* la) to_string() string {
    s := "Liveness Analysis:\n"
    for b := i32(0); b < la.num_blocks; b += 1 {
        s += "Block " + string(b) + ":\n"
        s += "  Gen set: "
