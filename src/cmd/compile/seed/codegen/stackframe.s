...
func (sf* stack_frame) allocate_local( var_name string, int size) int {
...
func stack_frame_emit_prologue(sf* stack_frame,codegen_context* ctx, string fn_name) {
...
func stack_frame_emit_epilogue(codegen_context* ctx) {
    ctx.emit_line("    leave")
    ctx.emit_line("    ret")
}

func stack_frame_emit_spill(codegen_context* ctx, string reg, int offset) {
    ctx.emit_line("    mov %" + reg + ", " + offset as string + "(%rbp)")
}

func stack_frame_emit_restore(codegen_context* ctx, string reg, int offset) {
