...
static bool write_runner_c_file(FILE *out, const char *ir_text, compile_error *err) {
	if (fprintf(out,
		...
		"    compile_error bootstrap_err;\n"
		"    error_clear(&bootstrap_err);\n"
		"    if (!seed_bootstrap_two_stage_check(compiler_src, out_dir, &bootstrap_err)) {\n"
		"        print_compile_error(&bootstrap_err);\n"
		"        return 1;\n"
		"    }\n"
		"    printf(\"bootstrap self-host check passed\\n\");\n"
		"    return 0;\n"
		"}\n\n") < 0) {
		error_set(err, ERR_SEMANTIC, 0, 0, "failed to write generated C source");
		return false;
