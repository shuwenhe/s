...
If you open the repository root directly, VS Code will not auto-load `/app/s/editor/vscode` as an installed extension. In that case the workspace can still associate `*.s` with the `s` language id, but syntax highlighting will only appear after this extension is launched in an Extension Development Host or installed as a VSIX.
