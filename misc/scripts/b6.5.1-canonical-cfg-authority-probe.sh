...

{
    ...
    echo "        string kind"
    echo "        option[mir_operand] condition"
    ...
    echo "    • finalized canonical MIR has mir_terminator.condition"
    echo "    • if lowering binds s.dump_expr(if_expr.condition.value) into branch_condition"
    echo "    • branch terminator stores condition + then edge + else edge together"
    echo ""
    echo "    Answer: YES"
    echo ""
    ...
    echo "  Q2: canonical-branch-condition-visible = YES ✓"
    ...
    echo "1. B6.5.1 GREEN:"
    echo "   canonical finalized MIR exposes condition + true target + false target"
    echo ""
