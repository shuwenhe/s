...
usage() {
    ...
    echo '  s --emit-mir-place-borrow <input.s> <output.mir>' >&2
...
if [ "$#" -eq 3 ] && [ "$1" = "--emit-mir-place-borrow" ]; then
...
if [ "$#" -eq 3 ] && [ "$1" = "--emit-mir-reference-liveness" ]; then
...
if [ "$#" -eq 3 ] && [ "$1" = "--emit-mir-loan-liveness" ]; then
...
if [ "$#" -eq 3 ] && [ "$1" = "--emit-mir-nll-shadow" ]; then
