...
"$S_COMPILER_ENV" build "$ROOT/$ENTRY" -o "$TMPDIR/s_compiler_stage1" || true
if [ -x "$TMPDIR/s_compiler_stage1" ]; then
  ...
  echo "stage1 compiler not produced; attempting to use initial compiler directly"
  export S_COMPILER="$S_COMPILER_ENV"
fi

echo "attempting self-hosted full build with S_COMPILER=$S_COMPILER"
"$ROOT/misc/scripts/install_selfhost_compiler_launcher.sh" "$TMPDIR/s_final_compiler" || true
echo "done"
