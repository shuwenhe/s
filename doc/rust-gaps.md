...
本次对照以本地 Rust 标准库、借用检查测试和 S 的可执行所有权编译路径为依据。
不是全仓库功能穷举。S 已有 `src/option`、`src/result`、`src/vec` 等源码；
不能把同名包存在等同于这些能力已进入所有权编译路径，也不能说 S 完全没有这些包。

| 优先级 | 能力 | 当前 S 所有权路径 | 价值 |
| --- | --- | --- | --- |
...
| 5 | 完整原生后端及自举闭环 | 当前编译器由 seed 构建，输出 C11 | 消除当前工具链边界并扩大可验证的平台覆盖 |
...
`src/cmd/compile/compiler/compiler.s` 增加子借用的父引用记录，并持续记录原始所有者。
...
运行 `make compiler-check` 构建 S 编译器并执行带 ASan/UBSan 的运行用例、
编译拒绝用例、算术异常及应用链接符号检查。支持范围详见 [compiler.md](compiler.md)。

## 本地对照入口

- Rust：`../rust/library/core/src/clone.rs` 中共享引用与可变引用的 Clone 实现。
- Rust：`../rust/tests/ui/borrowck/` 中重借用检查用例。
- S：`src/cmd/compile/compiler/compiler.s`、`test/compiler/check.py`。
